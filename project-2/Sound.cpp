...
namespace JSH
{
    HRESULT Sound::Load(const wstring& path)
    {
        if (nullptr == Soundmng::GetDevice())
    ...
    bool Sound::LoadWavFile(const wstring& path)
    {
        ...
        if (FAILED(Soundmng::GetDevice()->CreateSoundBuffer(&mBufferDesc, &mSoundBuffer, NULL)))
        {
